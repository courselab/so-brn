...
struct cmd_t cmds[] =
  {
    ...
    {"list",    f_list},     /* List content of disk */
    {0, 0}
  };
...
void f_help()
{
  ...
  kwrite ("      quit    (to exit iDOS)\n");
  kwrite ("      list    (list disk files)\n");
}
...
void f_list()
{
  fs_header *fs_h = (fs_header*)0x7c00;
  char *directory_region = (char*)((char*)fs_h + fs_h->number_of_boot_sectors * 512);
  for (int i =0; i < fs_h->number_of_file_entries; i++) {
    const char *entry = &directory_region[i * fs_h->max_file_size];
    kwrite(entry);
    kwrite("\n");
  }
  kwrite("\n");
